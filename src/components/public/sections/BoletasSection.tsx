...
export default function BoletasSection() {
    const getCalificacionColor = (calificacion: number) => {
        if (calificacion >= 9) return 'text-success bg-success/10';
        if (calificacion >= 8) return 'text-primary bg-primary-soft';
        if (calificacion >= 7) return 'text-warning-foreground bg-warning/15';
        return 'text-danger bg-danger/10';
    };

    const getPromedioColor = (promedio: number) => {
        if (promedio >= 9) return 'from-success to-success';
        if (promedio >= 8) return 'from-brand-500 to-brand-700';
        if (promedio >= 7) return 'from-brand-600 to-brand-800';
        return 'from-danger to-danger';
    };

    return (
        <section id="boletas" className="py-16 lg:py-24 bg-surface-app">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid gap-8 lg:grid-cols-2">
                    {boletas.map((boleta) => (
                        ...
                        >
                            {/* Header Card */}
                            <div className={`p-6 bg-gradient-to-r ${getPromedioColor(boleta.promedio)} text-primary-foreground`}>
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-xs text-primary-foreground/90">Promedio</div>
                                    </div>
                                </div>
                            ...
                            <div className="p-6">
                                ...
                                <div className="mt-6 pt-6 border-t border-border">
                                    ...
                                    <button className="w-full py-3 bg-primary hover:bg-primary-hover text-primary-foreground font-medium rounded-xl transition-colors duration-200 flex items-center justify-center gap-2">
