...
export type AttendanceSettingsPayload = {
    ...
    exit_earliest: string;
    entry_window_closes_at: string;
};

export async function getAttendanceSettings(): Promise<DailyAttendanceRules> {
    const response = await apiClient.get<{
        success: boolean;
...
): Promise<DailyAttendanceRules> {
    const response = await apiClient.put<{
        success: boolean;
        data: DailyAttendanceRules;
    }>(API_ENDPOINTS.ATTENDANCE.SETTINGS, payload);
