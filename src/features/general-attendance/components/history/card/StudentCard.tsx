import { DailyAttendanceStudent, CredentialLifecycleStatus, GeneralAttendanceStatus } from "@/features/general-attendance/types/general-attendance";
...
export default function StudentCard({ student }: StudentCardProps) {
  ...
  return (
    <div className={`border rounded-lg p-2 transition-shadow hover:shadow-md border-primary ${style.card}`}>
      <div className="flex items-start gap-1 xl:gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {(student.group || student.grade) && (
            <p className="mt-0.5 text-sm text-fg-muted flex items-center gap-1 justify-between ">
      ...
      <div className="mt-1 text-xs text-fg-muted min-h-4">
        {(entry || exit) && (
          <p>
            {entry && `Entrada ${entry}`}
            {entry && exit && " · "}
