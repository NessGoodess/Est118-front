...
/** Skeleton de /Announcement: header + botón crear + tabla. */
export default function AdminAnnouncementsListSkeleton({
  label = "Cargando avisos",
  rows = 6,
}: {
  label?: string
  rows?: number
    ...
    >
      <div className="mb-6 mt-4 flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-2">
          <SkeletonBone className="h-8 w-28 max-w-full" />
          <SkeletonBone className="h-4 w-80 max-w-full bg-surface-muted/80" />
        </div>
        <SkeletonBone className="h-10 w-36 shrink-0 rounded-xl" />
      </div>
