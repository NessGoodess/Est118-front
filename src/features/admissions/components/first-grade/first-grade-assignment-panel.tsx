...
import { useAdmissionIntakeSettings } from "@/features/admissions";
import { useAdmissionCapabilities } from "@/features/admissions/hooks/capabilities/useAdmissionCapabilities";
import { assignFirstGradeGroups } from "@/features/admissions/services/admissions.service";
import { handleApiError } from "@/lib/api";
import {
  FirstGradeAssignmentResult,
  ScoreSource,
} from "@/features/admissions/types/first-grade-assignment";
...
const scoreSourceLabel: Record<ScoreSource, string> = {
  ...
  combined: "Combinado (examen + promedio)",
};

const FLAG_SIBLINGS = "possible_siblings_by_lastname";
const FLAG_SCHOOL = "same_previous_school";
const FLAG_GUARDIAN = "same_guardian_curp";

function flagLabel(flag: string): string {
  if (flag === FLAG_SCHOOL) return "Misma escuela de procedencia";
  if (flag === FLAG_SIBLINGS) return "Posibles hermanos (apellido)";
  if (flag === FLAG_GUARDIAN) return "Mismo CURP de tutor";
  if (flag === "missing_score") return "Sin puntaje (no se asigna)";
  if (flag === "hard_constraint_unresolved") return "Sin grupo viable (restricción dura)";
  return flag;
}
...
export default function FirstGradeAssignmentPanel() {
  ...
  useEffect(() => {
    ...
    if (intakeSettings.score_mode === "exam") setScoreSource("admission_exam");
    else if (intakeSettings.score_mode === "combined") setScoreSource("combined");
    else setScoreSource("school_average");
  }, [intakeSettings]);
  ...
  return (
    <section className="bg-surface-elevated rounded-xl shadow-sm border border-border p-4 sm:p-6 space-y-4">
      <header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            disabled={yearsLoading || running || !canEditEnrollment}
        ...
        <div>
          ...
          <select
            className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm bg-surface-elevated"
            disabled={running || !canEditEnrollment}
            value={scoreSource}
          ...
          >
            ...
            <option value="combined">{scoreSourceLabel.combined}</option>
          </select>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        {canEditEnrollment ? (
          ...
          <p className="text-xs text-fg-muted">
            Solo consulta. No tienes permiso para simular ni aplicar la asignación de grupos.
          </p>
        )}
      </div>
                ...
                {filteredAssignments.map((item) => {
                  ...
                  return (
                    <tr key={item.enrollment_id} className="border-t border-border">
                      ...
                      <td className="px-3 py-2">
                        <select
                          className="rounded-lg border border-border px-2 py-1 text-xs bg-surface-elevated disabled:opacity-60"
                          value={selectedGroup ?? ""}
